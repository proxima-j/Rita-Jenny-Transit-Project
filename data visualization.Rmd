...
```{r}
gas %>% 
  mutate(price = `Weekly Minnesota Regular Conventional Retail Gasoline Prices Dollars per Gallon`,
         date = as.Date(`Week of`, "%m/%d/%Y")) %>% 
  filter(year(date)>=2014) %>% 
  filter(year(date)<2018) %>% 
  ggplot(aes(x = date, y = price))+
  geom_line()+
  labs(title = "Change of weekly gas price over time in Minnesota")
```
# Introduction
