...
write_csv(final_single_route, file = "finaldatasets/single_route.csv")
```
...
rider_gas_complete %>% 
  ...
  geom_text(aes(x = max_date, y = 2.4), label = "Gas Price",color = "#9F2C2C",size = 3)+
  geom_text(aes(x =  max_date, y = 1.7), label = "Total Riders",color = "#3F4345",size = 3)+
  theme_minimal()+
...
GRAPH1 <- rider_gas_complete %>% 
  ...
  mutate(meann = mean(avg_riders),
         norm_de_sea = (avg_riders-meann)/sd(avg_riders)/2.5+1.5) %>% 
  filter(Route == 63) %>% 
  # filter(year > 2014 & year < 2018) %>% 
  # filter(RouteClass == "CoreLoc") %>% 
  ggplot(aes(x = date))+
...
ggplotly(GRAPH1, tooltip = c("gas_price","date")) %>% 
